...
import { useGalleryUpload, useMediaDelete } from "@/shared/hooks/useMedia";
...
export function MediaGalleryField({
  ...
  const uploadMutation = useGalleryUpload(professionalId, {
    onSuccess: (media) => {
      setError(null);
  ...
  const removeMutation = useMediaDelete({
    onSuccess: (_data, id) => {
