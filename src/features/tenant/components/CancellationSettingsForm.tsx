...
import { useUpdateCancellationSettings } from "@/features/tenant/hooks/useCancellationSettings";
...
export function CancellationSettingsForm({
  ...
  const mutation = useUpdateCancellationSettings(tenantId, {
    onSuccess: () => {
      setError(null);
      setSaved(true);
      onSaved?.();
  ...
  return (
    <div className="rounded-2xl border border-border/50 bg-card p-6">
      ...
      <Button
        ...
        onClick={() => mutation.mutate({ enabled, leadMinutes: lead })}
      >
        {mutation.isPending ? (
