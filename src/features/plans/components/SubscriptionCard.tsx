...
import { SubscriptionStatus } from "@/features/plans/model/plans.types";
import { getErrorCode, formatApiError } from "@/shared/lib/api/errors";
...
export function SubscriptionCard({ tenantId }: { tenantId: string }) {
  ...
  const { data, isLoading, isError, error } = useSubscription(tenantId);
