...
import {
  ...
  useTenantReport,
  useExportReport,
} from "@/features/reports/hooks/useReportsPanel";
import { formatApiError, getErrorCode } from "@/shared/lib/api/errors";
import { formatPriceBRL, cn } from "@/shared/lib/utils";
import type {
  EliteReport,
  MonthlyMetrics,
  ProReport,
...
export function ReportsPanel({ tenantId }: { tenantId: string }) {
  ...
  const subQuery = useReportsSubscription(tenantId);
  ...
  const reportQuery = useTenantReport(
    tenantId,
    tier,
    canPickMonths ? months : null,
    !!subQuery.data && tier !== "NONE",
  );

  const exportMutation = useExportReport(tenantId, months);
