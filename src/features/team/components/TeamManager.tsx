...
import {
  useTeamMembers,
  useTeamInvitations,
  useOnboardTeamMember,
  useCancelTeamInvitation,
  useRemoveTeamMember,
} from "@/features/team/hooks/useTeamManager";
...
export function TeamManager({ tenantId, canManage }: TeamManagerProps) {
  ...
  const membersQuery = useTeamMembers(tenantId);

  const invitationsQuery = useTeamInvitations(tenantId, canManage);

  const onboardMutation = useOnboardTeamMember(tenantId, {
    onSuccess: (res) => {
      setError(null);
      setEmail("");
  ...
  const cancelMutation = useCancelTeamInvitation(tenantId, {
    onSuccess: () => {
      setError(null);
  ...
  function handleOnboard() {
    ...
    setFeedback(null);
    onboardMutation.mutate({ email: trimmed, role });
  }
