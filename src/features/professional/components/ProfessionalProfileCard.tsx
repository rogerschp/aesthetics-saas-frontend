...
import {
  ...
  useActivateProfessionalProfile,
} from "@/features/professional/hooks/useProfessionalProfileMutations";
...
import { maskPhoneBR, phoneToApiDigits } from "@/shared/lib/masks";
import { cn } from "@/shared/lib/utils";
import { MediaImageField } from "@/shared/components/MediaImageField";
import {
  MediaGalleryField,
  type GalleryItem,
} from "@/shared/components/MediaGalleryField";
...
export function ProfessionalProfileCard({
  ...
  const deactivateMutation = useDeactivateProfessionalProfile({
    onSuccess: () => {
      setError(null);
      setMode("view");
    },
  ...
  const activateMutation = useActivateProfessionalProfile({
    onSuccess: () => {
      setError(null);
      setMode("view");
    },
      ...
      <div className="rounded-2xl border border-border/50 bg-card p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <Button
            disabled={busy}
            onClick={() => {
              const payload = buildPayload(form, meQuery.data?.name ?? "");
