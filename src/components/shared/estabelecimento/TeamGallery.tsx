...
export function TeamGallery({ time, variante = "padrao" }: TeamGalleryProps) {
  ...
  // Variante Padrão: Scroll horizontal (original)
  if (variante === "padrao") {
    return (
  ...
  // Variante Alternativo: Grid com fotos grandes
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {time.map((membro) => (
            <div
              key={membro.id}
              className="flex items-center gap-4 bg-card border border-border/40 rounded-xl p-5 hover:border-primary/50 transition-colors group cursor-pointer"
            >
  ...
  // Variante Compacto: Lista vertical simples
  return (
    <div className="flex flex-col gap-4">
      <h3 className="text-lg font-semibold">Nossos Profissionais</h3>
      ...
      <div className="space-y-2">
        {time.map((membro) => (
          <div
            key={membro.id}
            className="flex items-center gap-3 py-2.5 px-3 rounded-lg hover:bg-card/60 transition-colors group cursor-pointer"
          >
